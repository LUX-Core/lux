...
class ExampleTest(LuxTestFramework):
    ...
    def run_test(self):
        ...
        w = 5.0
        while len([x for x in node0.rpc.listorderbook()['orders'] if x['status'] == 'contract_ack']) != 1 and w > 0.0:
            time.sleep(0.1)
            w -= 0.1
        if w <= 0: raise AssertionError("Timeout while waiting contract_ack order in orderbook")
