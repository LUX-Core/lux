...
#------------------------------------------------------------------------------
# Bash script for installing pre-requisite packages for cpp-cpp-ethereum on a
...
uname -v > /dev/null 2>&1 || { echo >&2 "ERROR - cpp-cpp-ethereum requires 'uname' to identify the platform."; exit 1; }
...
Darwin)
    case $(sw_vers -productVersion | awk -F . '{print $1"."$2}') in
        10.9)
            echo "Installing cpp-cpp-ethereum dependencies on OS X 10.9 Mavericks."
            ;;
        10.10)
            echo "Installing cpp-cpp-ethereum dependencies on OS X 10.10 Yosemite."
            ;;
        10.11)
            echo "Installing cpp-cpp-ethereum dependencies on OS X 10.11 El Capitan."
            ;;
        10.12)
            echo "Installing cpp-cpp-ethereum dependencies on macOS 10.12 Sierra."
            echo ""
            ...
            echo "known issues (see https://github.com/cpp-ethereum/webthree-umbrella/issues/614)."
    ...
    brew -v > /dev/null 2>&1 || { echo >&2 "ERROR - cpp-cpp-ethereum requires a Homebrew install.  See http://brew.sh."; exit 1; }
...
#------------------------------------------------------------------------------
FreeBSD)
    echo "Installing cpp-cpp-ethereum dependencies on FreeBSD."
    ...
    echo "At https://gitter.im/cpp-ethereum/cpp-cpp-ethereum-development."
    exit 1
    ;;
...
#------------------------------------------------------------------------------
    ...
    if [ -f "/etc/arch-release" ]; then
        ...
        echo "Installing cpp-cpp-ethereum dependencies on Arch Linux."
        ...
        Debian*)
            echo "Installing cpp-cpp-ethereum dependencies on Debian Linux."
        ...
        Fedora)
            echo "Installing cpp-cpp-ethereum dependencies on Fedora Linux."
            $SUDO dnf -qy install \
                gcc-c++ \
                leveldb-devel \
                curl-devel \
...
# See https://github.com/cpp-ethereum/webthree-umbrella/issues/228.
#------------------------------------------------------------------------------
        Ubuntu|"Linux Mint")
            echo "Installing cpp-cpp-ethereum dependencies on Ubuntu."
            if [ "$TRAVIS" ]; then
        ...
        CentOS*)
            echo "Installing cpp-cpp-ethereum dependencies on CentOS."
        ...
        *)
        ...
        # Alpine Linux
        echo "Installing cpp-cpp-ethereum dependencies on Alpine Linux."
...
#------------------------------------------------------------------------------
        openSUSE*)
            #openSUSE
            echo "Installing cpp-cpp-ethereum dependencies on openSUSE."
            echo "ERROR - 'install_deps.sh' doesn't have openSUSE support yet."
            echo "See http://cpp-cpp-ethereum.org/building-from-source/linux.html for manual instructions."
            echo "If you would like to get 'install_deps.sh' working for openSUSE, that would be fantastic."
            echo "See https://github.com/cpp-ethereum/webthree-umbrella/issues/552."
            exit 1
            ;;
...
#------------------------------------------------------------------------------
        *)
            ...
            echo "Drop us a message at https://gitter.im/cpp-ethereum/cpp-cpp-ethereum-development."
            exit 1
            ;;
        esac
    fi
...
*)
    ...
    echo "See http://cpp-cpp-ethereum.org/building-from-source/ for manual instructions."
