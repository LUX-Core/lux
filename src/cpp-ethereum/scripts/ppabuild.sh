...
##
## The "debian" directories can be found in github.com/cpp-ethereum/cpp-ethereum-ppa
...
pparepo=cpp-ethereum/cpp-ethereum
if [ -z "$codebranch" ]
then
	codebranch=develop
fi
...
then
    pparepo=cpp-ethereum/cpp-ethereum-dev
fi
echo codebranch=${codebranch}
echo pparepo=${pparepo}

keyid=703F83D0
email=builds@cpp-ethereum.org
mainppa="http://ppa.launchpad.net/cpp-ethereum/cpp-ethereum/ubuntu"
devppa="http://ppa.launchpad.net/cpp-ethereum/cpp-ethereum-dev/ubuntu"

# clone source repo
git clone https://github.com/cpp-ethereum/${mainrepo}.git -b ${codebranch} --recursive
...
wget https://github.com/cpp-ethereum/cpp-ethereum-ppa/archive/${ppabranch}.tar.gz -O- |
...
# do the build
#pdebuild --buildresult . --pbuilder /usr/sbin/cowbuilder --architecture amd64 -- --buildresult . --basepath /var/cache/pbuilder/${distribution}-${arch}-cpp-ethereum.cow
