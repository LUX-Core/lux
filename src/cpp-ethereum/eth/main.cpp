...
#include <libcpp-ethereum/Defaults.h>
#include <libcpp-ethereum/BlockChainSync.h>
...
//		c_gasFloorTarget = gasFloor;
	...
	if (g_logVerbosity > 0)
		cout << EthGrayBold "cpp-cpp-ethereum, a C++ Ethereum client" EthReset << endl;
	...
	if (!extraData.empty())
		web3.cpp-ethereum()->setExtraData(extraData);

	auto toNumber = [&](string const& s) -> unsigned {
		if (s == "latest")
			return web3.cpp-ethereum()->number();
		if (s.size() == 64 || (s.size() == 66 && s.substr(0, 2) == "0x"))
			return web3.cpp-ethereum()->blockChain().number(h256(s));
		try {
			return stol(s);
		}
		catch (...)
	...
	if (mode == OperationMode::Export)
	{
		...
		{
			bytes block = web3.cpp-ethereum()->blockChain().block(web3.cpp-ethereum()->blockChain().numberHash(i));
			switch (exportFormat)
			{
	...
	if (mode == OperationMode::Import)
	{
		...
		while (in.peek() != -1)
		{
			...
			switch (web3.cpp-ethereum()->queueBlock(block, safeImport))
			{
			...
			tuple<ImportRoute, bool, unsigned> r = web3.cpp-ethereum()->syncQueue(10);
			imported += get<2>(r);
			...
			if ((unsigned)e >= last + 10)
			{
				...
				auto d = e - last;
				cout << i << " more imported at " << (round(i * 10 / d) / 10) << " blocks/s. " << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.cpp-ethereum()->number() << ")" << endl;
				last = (unsigned)e;
				lastImported = imported;
//				cout << web3.cpp-ethereum()->blockQueueStatus() << endl;
			}
		}
		...
		{
			this_thread::sleep_for(chrono::seconds(1));
			tie(ignore, moreToImport, ignore) = web3.cpp-ethereum()->syncQueue(100000);
		}
		double e = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - t).count() / 1000.0;
		cout << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.cpp-ethereum()->number() << ")" << endl;
		return 0;
	}
			...
			if (!keyManager.load(masterPassword) && masterSet)
			{
				while (true)
				{
					...
					if (keyManager.load(masterPassword))
						break;
					cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " + getDataDir("cpp-ethereum") + "/keys.info" << endl;
				}
			}
		}
		else
...
//	std::shared_ptr<eth::BasicGasPricer> gasPricer = make_shared<eth::BasicGasPricer>(u256(double(ether / 1000) / etherPrice), u256(blockFees * 1000));
	...
	eth::Client* c = nodeMode == NodeMode::Full ? web3.cpp-ethereum() : nullptr;
	if (c)
	{
	...
	if (testingMode)
		...
		authenticator = [&](TransactionSkeleton const& _t, bool isProxy) -> bool {
			...
			string r = getResponse(_t.userReadable(isProxy,
				...
				{
					h256 contractCodeHash = web3.cpp-ethereum()->postState().codeHash(_t.to);
	...
	if (jsonRPCURL > -1 || ipc)
	{
		...
		accountHolder.reset(new SimpleAccountHolder([&](){ return web3.cpp-ethereum(); }, getAccountPassword, keyManager, authenticator));
		auto ethFace = new rpc::Eth(*web3.cpp-ethereum(), *accountHolder.get());
		rpc::TestFace* testEth = nullptr;
		if (testingMode)
			testEth = new rpc::Test(*web3.cpp-ethereum());
		...
		if (jsonRPCURL >= 0)
		{
			...
			if (adminViaHttp)
			{
				personal = new rpc::Personal(keyManager, *accountHolder, *web3.cpp-ethereum());
				adminEth = new rpc::AdminEth(*web3.cpp-ethereum(), *gasPricer.get(), keyManager, *sessionManager.get());
			...
			jsonrpcHttpServer.reset(new FullServer(
				...
				new rpc::Debug(*web3.cpp-ethereum()),
				testEth
			));
		...
		if (ipc)
		{
			jsonrpcIpcServer.reset(new FullServer(
				...
				new rpc::Web3(web3.clientVersion()), new rpc::Personal(keyManager, *accountHolder, *web3.cpp-ethereum()),
				new rpc::AdminEth(*web3.cpp-ethereum(), *gasPricer.get(), keyManager, *sessionManager.get()),
