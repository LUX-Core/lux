...
// /home/gav/Eth/cpp-cpp-ethereum/test/jsonrpc.cpp(169): last checkpoint
#if ETH_JSONRPC && 0
...
struct Setup
{
	Setup()
	{
		...
		web3->cpp-ethereum()->setForceMining(true);
...
BOOST_AUTO_TEST_CASE(jsonrpc_defaultBlock)
{
	...
	BOOST_CHECK_EQUAL(defaultBlock, web3->cpp-ethereum()->getDefault());
}
...
BOOST_AUTO_TEST_CASE(jsonrpc_isMining)
{
	...
	web3->cpp-ethereum()->startSealing();
	bool miningOn = jsonrpcClient->eth_mining();
	BOOST_CHECK_EQUAL(miningOn, web3->cpp-ethereum()->isMining());

	web3->cpp-ethereum()->stopSealing();
	bool miningOff = jsonrpcClient->eth_mining();
	BOOST_CHECK_EQUAL(miningOff, web3->cpp-ethereum()->isMining());
}
...
BOOST_AUTO_TEST_CASE(jsonrpc_number)
{
	...
	BOOST_CHECK_EQUAL(number, web3->cpp-ethereum()->number() + 1);
	dev::eth::mine(*(web3->cpp-ethereum()), 1);
	int numberAfter = jsonrpcClient->eth_number();
	BOOST_CHECK_EQUAL(number + 1, numberAfter);
	BOOST_CHECK_EQUAL(numberAfter, web3->cpp-ethereum()->number() + 1);
}
...
BOOST_AUTO_TEST_CASE(jsonrpc_setMining)
{
	...
	BOOST_CHECK_EQUAL(web3->cpp-ethereum()->isMining(), true);

	jsonrpcClient->eth_setMining(false);
	BOOST_CHECK_EQUAL(web3->cpp-ethereum()->isMining(), false);
}
...
BOOST_AUTO_TEST_CASE(jsonrpc_stateAt)
{
	...
	BOOST_CHECK_EQUAL(toJS(web3->cpp-ethereum()->stateAt(address, jsToU256("0"), 0)), stateAt);
}
...
BOOST_AUTO_TEST_CASE(jsonrpc_transact)
{
	...
	BOOST_CHECK_EQUAL(jsToAddress(coinbase), web3->cpp-ethereum()->author());
	
	dev::KeyPair key = KeyPair::create();
	auto address = key.address();
	auto receiver = KeyPair::create();
	web3->cpp-ethereum()->setAuthor(address);
	...
	auto balance2 = web3->cpp-ethereum()->balanceAt(receiver.address());
...
BOOST_AUTO_TEST_CASE(simple_contract)
{
	...
	web3->cpp-ethereum()->setAuthor(kp.address());
