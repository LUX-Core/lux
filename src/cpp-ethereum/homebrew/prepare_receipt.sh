...
rm -rf cpp-cpp-ethereum
mkdir cpp-cpp-ethereum
mkdir cpp-cpp-ethereum/$VERSION
p="../webthree-helpers/homebrew/"
cp ${p}homebrew.mxcl.cpp-cpp-ethereum.plist ${p}INSTALL_RECEIPT.json ../web-helpers/LICENSE cpp-cpp-ethereum/$VERSION
...
mkdir -p install
cp -rf install/* cpp-cpp-ethereum/$VERSION

# tar everything
NAME="cpp-cpp-ethereum-${VERSION}.${OSX_VERSION}.bottle.${NUMBER}.tar.gz"
tar -zcvf $NAME cpp-cpp-ethereum

# get variables
HASH=`git rev-parse HEAD`
SIGNATURE=`openssl sha1 ${NAME} | cut -d " " -f 2`

# Pull the current cpp-cpp-ethereum.rb file from Github.  We used to use a template file.
curl https://raw.githubusercontent.com/cpp-ethereum/homebrew-cpp-ethereum/master/cpp-cpp-ethereum.rb > cpp-cpp-ethereum.rb.in
...
if [ ${OSX_VERSION} == yosemite ]; then
    sed -e s/revision\ \=\>\ \'[[:xdigit:]][[:xdigit:]]*\'/revision\ \=\>\ \'${HASH}\'/g \
        ...
        -e s/revision[[:space:]][[:digit:]][[:digit:]]*/revision\ ${NUMBER}/g < cpp-cpp-ethereum.rb.in > "cpp-cpp-ethereum.rb"
else
    sed -e s/revision\ \=\>\ \'[[:xdigit:]][[:xdigit:]]*\'/revision\ \=\>\ \'${HASH}\'/g \
        -e s/version\ \'.*\'/version\ \'${VERSION}\'/g \
