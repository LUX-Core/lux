...
#include <libcpp-ethereum/Client.h>
#include <libcpp-ethereum/ChainParams.h>
...
public:
	...
	/// cpp-ethereum() may be safely static_cast()ed to a eth::Client*.
	WebThreeDirect(
	...
	eth::Client* cpp-ethereum() const { if (!m_cpp-ethereum) BOOST_THROW_EXCEPTION(InterfaceNotSupported("eth")); return m_cpp-ethereum.get(); }
	...
	u256 networkId() const override { return m_cpp-ethereum.get()->networkId(); }
...
private:
	...
	std::unique_ptr<eth::Client> m_cpp-ethereum;		///< Client for Ethereum ("eth") protocol.
...
public:
	...
	eth::Interface* cpp-ethereum() const { if (!m_cpp-ethereum) BOOST_THROW_EXCEPTION(InterfaceNotSupported("eth")); return m_cpp-ethereum; }
...
private:
	EthereumSlave* m_cpp-ethereum = nullptr;
