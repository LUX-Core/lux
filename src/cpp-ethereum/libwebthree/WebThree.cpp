...
#include <libcpp-ethereum/EthereumHost.h>
#include <libcpp-ethereum/ClientTest.h>
	...
	m_net(_clientVersion, _n, _network)
{
	...
	if (_interfaces.count("eth"))
	{
		...
		if (_params.sealEngineName == "Ethash")
			m_cpp-ethereum.reset(new eth::EthashClient(_params, (int)_params.u256Param("networkID"), &m_net, shared_ptr<GasPricer>(), _dbPath, _we));
		else if (_params.sealEngineName == "NoProof" && _testing)
			m_cpp-ethereum.reset(new eth::ClientTest(_params, (int)_params.u256Param("networkID"), &m_net, shared_ptr<GasPricer>(), _dbPath, _we));
		else
			m_cpp-ethereum.reset(new eth::Client(_params, (int)_params.u256Param("networkID"), &m_net, shared_ptr<GasPricer>(), _dbPath, _we));
		...
		m_cpp-ethereum->setExtraData(rlpList(0, string(dev::Version) + "++" + string(DEV_QUOTED(ETH_COMMIT_HASH)).substr(0, 4) + (ETH_CLEAN_REPO ? "-" : "*") + string(DEV_QUOTED(ETH_BUILD_TYPE)).substr(0, 1) + boost::join(bps, "/")));
	}
...
WebThreeDirect::~WebThreeDirect()
{
	...
	// still referencing Sessions getting deleted *after* m_cpp-ethereum is reset, causing bad things to happen, since
	// the guarantee is that m_cpp-ethereum is only reset *after* all sessions have ended (sessions are allowed to
	// use bits of data owned by m_cpp-ethereum).
	m_net.stop();
	m_cpp-ethereum.reset();
}
