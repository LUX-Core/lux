...
public:
    ...
    bool SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nLuxsendRoundsMin, int nLuxsendRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nLuxsendRoundsMin, int nLuxsendRoundsMax);
    ...
    int GenerateLuxsendOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the Luxsend chain depth for a given input
    int GetRealInputLuxsendRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputLuxsendRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            const int rounds = pwallet->GetInputLuxsendRounds(vin);
            if (rounds >= -2 && rounds < nLuxsendRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with Luxsend. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
