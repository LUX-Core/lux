...
UniValue spork(const JSONRPCRequest& request)
{
    ...
    throw std::runtime_error(
        "spork <name> [<value>]\n"
        "\nSets or returns value of the spork.\n"
        "\nArguments:\n"
        "1. name    (string, required) The corresponding spork name, or 'show' to show all current spork settings, active to show which sporks are active\n"
        "2. value   (number, optional) An epoch datetime to enable or disable spork\n" +
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "to the wallet before upgrading. To use the deprecated functionality, start luxd with -deprecatedrpc=createmultisig\n"
            "\nArguments:\n"
