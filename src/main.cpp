    ...
    if (fTxIndex) {
        ...
        if (pblocktree->ReadTxIndex(hash, postx)) {
            ...
            if (!pindexPrev)
                ...
                hashBlock = header.GetHash(pindexPrev->nHeight + 1);
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (pindex->nHeight >= Params().FirstSCBlock() && !IsTestNet()) {
        ...
        if ((checkBlock.GetHash(pindex->nHeight) != block.GetHash(pindex->nHeight)) &&
            (block.nVersion & (1 << 30)) &&
            (block.hashStateRoot != uint256() &&
             block.hashUTXORoot != uint256()) &&
             !fJustCheck) {
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                hash = block.GetHash(pindexPrev ? pindexPrev->nHeight + 1 : 0);
