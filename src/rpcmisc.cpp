...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"luxsend_balance\": xxxxxx, (numeric) the anonymized lux balance of the wallet\n"
    ...
    if (pwalletMain) {
        ...
        if (!fLiteMode)
            obj.push_back(Pair("luxsend_balance", ValueFromAmount(pwalletMain->GetAnonymizedBalance())));
    }
#endif
