...
if (ui->treeWidget->topLevelItemCount() > 49){ //check we have more then 50 inputs 
        ...
        Qt::CheckState state = Qt::Checked; //set the state to checked so when (un)select is clicked all checkboxes will be unchecked  
            ui->treeWidget->setEnabled(false);
                for (int i = 0; i < 50; i++)
                    if (ui->treeWidget->topLevelItem(i)->checkState(COLUMN_CHECKBOX) != state)
                    ui->treeWidget->topLevelItem(i)->setCheckState(COLUMN_CHECKBOX, state);
        ...
        } else { // if we are not in list view give the user dialogue to inform them of this issue
            QMessageBox msgBox;
            msgBox.setObjectName("lockMessageBox");
            msgBox.setStyleSheet(GUIUtil::loadStyleSheet());
