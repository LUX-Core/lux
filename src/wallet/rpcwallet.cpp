...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\", \"tabby\"")
        );
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("getaccount", "\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\"")
            + HelpExampleRpc("getaccount", "\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\"")
        );
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        for (const CTxOut& txout : wtx.tx->vout)
            if (txout.scriptPubKey == scriptPubKey)
                if (wtx.GetDepthInMainChain(false) >= nMinDepth)
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            + HelpExampleCli("sendmany", "\"\" \"{\\\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\\\":0.01,\\\"LcnqiuG9q6WS5K6dR5tYtthMfJh9Uxopxg\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"\" \"{\\\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\\\":0.01,\\\"LcnqiuG9q6WS5K6dR5tYtthMfJh9Uxopxg\\\":0.02}\" 6 \"testing\"") +
            "\nSend two amounts to two different addresses, subtract fee from amount:\n"
            + HelpExampleCli("sendmany", "\"\" \"{\\\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\\\":0.01,\\\"LcnqiuG9q6WS5K6dR5tYtthMfJh9Uxopxg\\\":0.02}\" 1 \"\" \"[\\\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\\\",\\\"LcnqiuG9q6WS5K6dR5tYtthMfJh9Uxopxg\\\"]\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"\", {\"LgAskSorXfCYUweZcCTpGNtpcFotS2rqDF\":0.01,\"LcnqiuG9q6WS5K6dR5tYtthMfJh9Uxopxg\":0.02}, 6, \"testing\"")
        );
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        int nDepth = wtx.GetDepthInMainChain(true);
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    // Received
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain(false) >= nMinDepth)
    {
        bool stop = false;
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            + HelpExampleCli("listsinceblock", "\"3efa8c583a6b3f17b1f424a86c5ae65d98baff0292b760c9e64951f1823abfbd\" 6")
            + HelpExampleRpc("listsinceblock", "\"3efa8c583a6b3f17b1f424a86c5ae65d98baff0292b760c9e64951f1823abfbd\", 6")
        );
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("gettransaction", "\"3efa8c583a6b3f17b1f424a86c5ae65d98baff0292b760c9e64951f1823abfbd\"")
            + HelpExampleCli("gettransaction", "\"3efa8c583a6b3f17b1f424a86c5ae65d98baff0292b760c9e64951f1823abfbd\" true")
            + HelpExampleRpc("gettransaction", "\"3efa8c583a6b3f17b1f424a86c5ae65d98baff0292b760c9e64951f1823abfbd\"")
        );
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (fEnable) {
        if (request.params.size() != 2)
            throw std::runtime_error("Input Error: use format: autocombinerewards <true/false> threshold\n");
...
UniValue printMultiSend()
{
    ...
    for (unsigned int i = 0; i < pwallet->vMultiSend.size(); i++) {
        vMS.push_back(Pair("Address " + boost::lexical_cast<std::string>(i), pwallet->vMultiSend[i].first));
...
UniValue multisend(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() == 1) {
        ...
        if (strCommand == "print") {
            ...
            if (IsValidDestination(DecodeDestination(pwallet->vMultiSend[0].first))) {
...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "4. \"senderaddress\" (string, optional) The Lux address that will be used to create the contract.\n"
    ...
    if (!pwallet->CreateTransaction(vecSend, wtx, reservekey, nFeeRequired, strError, &coinControl,  ALL_COINS,
    ...
    ExtractDestination(pwallet->mapWallet[wtx.vin[0].prevout.hash].vout[wtx.vin[0].prevout.n].scriptPubKey,txSenderDest);
...
{ //  category              name                        actor (function)           argNames
    ...
    {"wallet",              "callcontract",             &callcontract,             {"address", "data", "address", "gasLimit"} },
