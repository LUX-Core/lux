...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    ...
    if (!ReadBlockFromDisk(block, blockPos, consensusParams))
        return false;
    if (block.GetHash(nHeight >= Params().SwitchPhi2Block()) != pindex->GetBlockHash())
    ...
    do {
...

        {
            ...
            do {
                ...
                if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetHash(usePhi2) == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))
                    return false;
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
                            LogPrint(BCLog::REINDEX, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash(usePhi2).ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                            ...
                            {
                                nLoaded++;
                                queue.push_back(pblockrecursive->GetHash(usePhi2));
                            }
                        }
                        range.first++;
