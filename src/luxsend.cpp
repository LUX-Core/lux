    ...
    if (strCommand == "dsa") { //Luxsend Accept Into Pool
        ...
        if (dsq.ready) {
            ...
            if (state == POOL_STATUS_QUEUE) {
                LogPrint("luxsend", "Luxsend queue is ready - %s\n", addr.ToString());
...
void CLuxsendPool::Check()
{
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 16120) {
...
void CLuxsendPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        LogPrintf("CLuxsendPool::Check() -- IS MASTER -- TRANSMITTING OBFUSCATION\n");
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 LUX per transaction on average.
            */
            if (r <= 10) {
...
bool CLuxsendPool::IsCollateralValid(const CTransaction& txCollateral)
{
...

    {
        ...
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            if (fDebug) LogPrintf("CLuxsendPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CLuxsendPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, int& errorID)
{
...
bool CLuxsendPool::AddScriptSig(const CTxIn& newVin)
{
    ...
    BOOST_FOREACH (CTxIn& vin, finalTransaction.vin) {
        if (newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence) {
            ...
            LogPrint("luxsend", "CObfuScationPool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0, 24));
        }
    }
    for (unsigned int i = 0; i < entries.size(); i++) {
        if (entries[i].AddSig(newVin)) {
            LogPrint("luxsend", "CObfuScationPool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0, 24));
            return true;
        }
    }
...
bool CLuxsendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    if (fMasterNode) return false;
...
bool CLuxsendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    ...
    LogPrintf("CLuxsendPool::SignFinalTransaction %s", finalTransaction.ToString());
...
bool CLuxsendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeLuxAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CLuxsendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, int& errorID)
{
...
std::string CLuxsendPool::GetMessageByID(int messageID)
{
    ...
    case ERR_MAXIMUM:
        return _("Value more than Luxsend pool maximum allows.");
    case ERR_MN_LIST:
    ...
    case ERR_RECENT:
        return _("Last Luxsend was too recent.");
    case ERR_SESSION:
...
void CLuxsendPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void CLuxsendPool::RelayCompletedTransaction(const int sessionID, const bool error, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
